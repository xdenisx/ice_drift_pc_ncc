    ...
    for fname in files:
        if fname.endswith('tiff'):
            ...
            dt_f = datetime.strptime(('%s/%s/%s' %
                                     (m[0:4], m[4:6], m[6:8])),
                                    '%Y/%m/%d')
            if not dt_f in unq_dates:
                unq_dates.append(dt_f)
...
for idate in unq_dates[1:]:
    ...
    if len(files_to_mosaic)>1:
        ...
        out_path_byte = '%s/ps_%dd_S1_mos_%s.tif' % (out_path, days, out_title)
        ...
        # Delete old files
        try:
            os.remove(out_path_byte)
        except:
            pass

        gtiff_enhance(tif_path, out_path_byte)
        print('Done.\n')
